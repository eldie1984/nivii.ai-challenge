...
router.get('/portfolio/:portfolioId', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    const result = await db.query(`SELECT t.*, i.symbol, i.name, i.type
     FROM transaction_service.transactions t
     JOIN portfolio_service.instruments i ON t.instrument_id = i.id
     WHERE t.portfolio_id = $1
     ORDER BY t.transaction_date DESC`, [portfolioId]);
    res.json({
        success: true,
        data: result.rows
    });
...
router.post('/', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const userId = req.headers['x-user-id'];
    const { portfolioId, instrumentId, type, quantity, price, brokerCommission = 0, exchangeCommission = 0, transactionTariff = 0, transactionDate, notes } = req.body;
    if (!portfolioId || !instrumentId || !type || !quantity || !price) {
        throw (0, errorHandler_1.createError)('Missing required fields', 400);
    }
    if (!['buy', 'sell'].includes(type)) {
        throw (0, errorHandler_1.createError)('Invalid transaction type', 400);
    }
    const totalAmount = Math.abs(parseFloat(quantity) * parseFloat(price));
    ...
    // Create transaction
    const transactionResult = await db.query(`INSERT INTO transaction_service.transactions 
     ...
     RETURNING *`, [portfolioId, instrumentId, type, quantity, price, totalAmount,
...
async function updateHoldings(db, portfolioId, instrumentId, quantity, price, type, totalAmount) {
    // Check if holding exists
    const existingHolding = await db.query('SELECT * FROM portfolio_service.holdings WHERE portfolio_id = $1 AND instrument_id = $2', [portfolioId, instrumentId]);
    if (existingHolding.rows.length > 0) {
        ...
        if (newQuantity <= 0) {
            ...
            await db.query('DELETE FROM portfolio_service.holdings WHERE portfolio_id = $1 AND instrument_id = $2', [portfolioId, instrumentId]);
        }
        else {
            // Update holding
            await db.query(`UPDATE portfolio_service.holdings 
         SET quantity = $1, average_cost = $2, updated_at = CURRENT_TIMESTAMP
         WHERE portfolio_id = $3 AND instrument_id = $4`, [newQuantity, newAverageCost, portfolioId, instrumentId]);
        }
    }
    else if (type === 'buy') {
        // Create new holding for first purchase
        await db.query(`INSERT INTO portfolio_service.holdings 
       (portfolio_id, instrument_id, quantity, average_cost, current_price)
       VALUES ($1, $2, $3, $4, $5)`, [portfolioId, instrumentId, quantity, price, price]);
    }
}
